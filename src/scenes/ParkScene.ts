...
export class ParkScene {
    ...
    CreateSkybox(): void {
        ...
        const glow = new GlowLayer("lampGlow", this.scene);
        glow.intensity = 0.3;
    }
    ...
    async CreateEnvironment(): Promise<void> {
        ...
        meshes.map((mesh) => {
            ...
            if (mesh.name === "GroundRight" || mesh.name === "GroundLeft") {
                ...
                if (material) {
                    if (material instanceof PBRMaterial || material instanceof StandardMaterial) {
                        (material as any).maxSimultaneousLights = 100;
                    }
                }
            ...
            if (mesh.name === "GroundMiddle") {
                ...
                pbr.albedoColor = new Color3(2.5, 2.5, 2.5);
                (pbr.albedoTexture as Texture).uScale = uScale;
                (pbr.albedoTexture as Texture).vScale = vScale;

                pbr.bumpTexture = new Texture("/textures/pavement_1/normal.jpg", this.scene);
                (pbr.bumpTexture as Texture).uScale = uScale;
                (pbr.bumpTexture as Texture).vScale = vScale;

                pbr.metallicTexture = new Texture("/textures/pavement_1/ao.jpg", this.scene);
                (pbr.metallicTexture as Texture).uScale = uScale;
                (pbr.metallicTexture as Texture).vScale = vScale;
